...
public class InitializerSeeder implements CommandLineRunner {
	...
	private void createOrganization() {
		organizationRepository.save(new OrganizationEntity("OrganizationTest", "image.jpg", "orgEmail@mail.com",
				"welcomeText", new Timestamp(System.currentTimeMillis())));
	}

	private void createNews() {
		newsRepository.save(new NewsEntity("NewsTest", "content", "img", categoryRepository.getById(6L),
				new Timestamp(System.currentTimeMillis()), false, "type"));
	}

	private void createCategory() {
		categoryRepository.save(new CategoryEntity("CategoryTest", "seeder category", "img",
	...
	private void createUsers(int users, String userType) {
		...
		if (userType.equalsIgnoreCase("admin")) {
			IntStream.range(0, users).forEach(userNumber -> createAdmin(userType, PASSWORD, userNumber));
		} else {
			IntStream.range(0, users).forEach(userNumber -> createNormalUser(userType, PASSWORD, userNumber));
		}
	}
